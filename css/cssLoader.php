...
if($pexeto_css['secondary_color']!=''){
	echo 'a.read-more, .no-caps{color:#'.$pexeto_css['secondary_color'].';}';
	echo '.button, #submit, input[type=submit], td#today, table#wp-calendar td:hover, table#wp-calendar td#today, table#wp-calendar td:hover a, table#wp-calendar td#today a{background-color:#'.$pexeto_css['secondary_color'].';}';
}
...
if($pexeto_css['logo_width']!=''){
	echo '#logo-container a img{max-width:'.$pexeto_css['logo_width'].'px;}';
}


if($pexeto_css['logo_height']!=''){
	echo '#logo-container a img{max-height:'.$pexeto_css['logo_height'].'px;}';
}
...
if($pexeto_css['link_color']!=''){
	echo 'a,.post-info, .post-info a, #main-container .sidebar-box ul li a{color:#'.$pexeto_css['link_color'].';}';
}

if($pexeto_css['heading_color']!=''){
	echo 'h1,h2,h3,h4,h5,h6,.sidebar-box h4,.post h1, .blog-post h1 a, .content-box h2, #portfolio-categories ul li, h1 a, h2 a, h3 a, h4 a, h5 a, h6 a, .services-box h4, #intro h1, #page-title h1, .item-desc h4 a, .item-desc h4, .sidebar-post-wrapper h6 a, table th, .tabs a, .post-title a:hover{color:#'.$pexeto_css['heading_color'].';}';
}
...
if($pexeto_css['footer_text_color']!=''){
	echo '#footer,#footer ul li a,#footer ul li a:hover,#footer h4, .copyrights{color:#'.$pexeto_css['footer_text_color'].';}';
}
...
if($pexeto_css['heading_font_family']!=''){
	echo 'h1,h2,h3,h4,h5,h6,.accordion-description a,#content-container .wp-pagenavi,#portfolio-categories ul li.selected,.table-title td,.table-description strong,table th,.tabs a{font-family:'.$pexeto_css['heading_font_family'].';}';
}

if($pexeto_css['body_font_family']!=''){
	echo 'body, .content-box .post-info{font-family:'.$pexeto_css['body_font_family'].';}';
}
